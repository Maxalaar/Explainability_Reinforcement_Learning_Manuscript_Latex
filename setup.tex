...
% black/red text stays legible on top of it.
\def\HeatCellMaxPct{55}
\setlength{\fboxsep}{1pt}

% #1 = condition (0 or 1), #2 = one-argument wrapping macro, #3 = content
\newcommand{\wrapif}[3]{\ifnum#1=1 #2{#3}\else#3\fi}

% A data cell: its background is a genuine function of its value (the closer
% to 1, the more saturated), computed here via pgfmath rather than
% precomputed -- #1 = value, #2 = is column-best (0/1), #3 = is row-best
% (0/1), #4 = is table-best (0/1).
\newcommand{\DataCell}[4]{%
  \pgfmathtruncatemacro{\heatcellpct}{round(max(0,min(1,#1))*\HeatCellMaxPct)}%
  \colorbox{myblue!\heatcellpct}{%
    \wrapif{#4}{\textcolor{red}}{\wrapif{#2}{\textbf}{\wrapif{#3}{\underline}{#1}}}%
  }%
}
