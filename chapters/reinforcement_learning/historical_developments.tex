...
The next step consisted in replacing such approximators with \gls{nn}, which would soon reshape the field.
...
This made it possible to train an agent to play Atari games at a human level directly from raw pixels \cite{mnih2013playingatarideepreinforcement}.
The use of deep \gls{nn} provided \gls{rl} with a powerful and flexible function approximator.
...
\gls{ppo} later became a widely adopted algorithm in \gls{rl} due to its simplicity, stability, and ability to handle both continuous and discrete action spaces \cite{schulman2017proximalpolicyoptimizationalgorithms}.
