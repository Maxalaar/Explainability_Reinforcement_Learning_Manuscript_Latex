...
\begin{enumerate}
    ...
    \item We define the optimal policy in Subsection~\ref{subsec:optimal_policy}, which is exactly what we seek to obtain when applying reinforcement learning.
\end{enumerate}
...
To formally specify both the task and the agent's capacity to perceive and act, we model the problem as a Markov Decision Process (MDP).
...
However, our focus here is to represent reinforcement learning as it is applied empirically, even if this means sacrificing some mathematical proof capability.  
...
Second, a terminal trajectory that starts from an initial state and an initial observation is called an episode, and is denoted by \(t_e \sim \tau(s^i, o, p, \pi)\).
...
This yields the compact notation \(V_\pi(s, o)\).
...
This function is called the state-action value function \(Q_\pi\), and it estimates the expected return when starting from a state \(s\), taking an action \(a\), and then following the policy \(\pi\) for all subsequent steps.
...
Recall that the goal of reinforcement learning is to find a policy that maximizes, for every observation, the expected cumulative reward until a terminal state is reached.  
