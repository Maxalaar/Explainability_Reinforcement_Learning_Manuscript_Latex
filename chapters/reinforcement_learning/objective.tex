...
\label{sec:rl_problem_formulation}

Having outlined the main developments shaping \gls{rl} and introduced the concept of the agent, the \gls{rl} problem can now be stated formally.
In essence, \gls{rl} aims to build a agent that makes sequential decisions in order to maximize a cumulative reward signal over the long term.
The purpose of this section is to translate this intuitive goal into a mathematical framework.
To this end, the presentation proceeds step by step:
\begin{enumerate}
    \item Modeling the \gls{rl} problem as a Markov Decision Process.
...
\begin{itemize}
    ...
    Unlike the state \(s \in \mathcal{S}\), which is hidden from the agent, the observation \(o \in \mathcal{O}\) provides partial information about the current state.
