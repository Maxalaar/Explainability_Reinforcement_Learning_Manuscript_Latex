...
\begin{enumerate}
    \item Modeling the reinforcement learning problem as a Markov Decision Process.
...
The \gls{rl} framework is grounded in the agent paradigm: the decision-making entity is called the agent.  
...
The following section aims to provide a mathematical description of the \gls{rl} problem.
The formulation adopted in this manuscript has been chosen to emphasize the main elements that will be used throughout the remainder of the manuscript.
...
They are jointly generated via \((s', o, r) \sim p(s, a)\), which implicitly defines the relationship between them.
In our \gls{mdp} formalization, this relationship is left implicit because it is never manipulated directly during learning.

However, to express mathematical equations, it is convenient to introduce the conditional distribution over states given an observation.
We denote this distribution by
\(b : \mathcal{O} \rightarrow \mathcal{P}(\mathcal{S}).\)
The notation \(b(s \mid o)\) denotes the probability of state \(s\) given the observation \(o\), while \(s \sim b(\cdot \mid o)\) indicates that the state \(s\) is sampled from this distribution.

\subsection{Policy}
\label{subsec:policy}

The agent’s decision-making capability is modeled by a policy function.  
The policy defines the stochastic mapping between observations \(o \in \mathcal{O}\) and actions \(a \in \mathcal{A}\).  
...
It is important to note that this policy does not include any explicit state-estimation mechanism.
Instead, the ability to infer information about the underlying state is entirely left to the policy itself, which must learn from experience how to compensate for the missing information.
This reflects the design of many modern \gls{rl} implementations, where policies are often parameterized by architectures capable of summarizing past observations, such as recurrent or attention-based networks, without explicitly modeling the uncertainty over the underlying state.

Now that the \gls{mdp} and the policy have both been defined, Figure~\ref{fig:reinforcement_learning_loop} illustrates how they interact within the general \gls{rl} loop.
As a reminder, the objective of \gls{rl} is to iteratively improve the policy so as to maximize the reward function.
...
\[
g(t, \gamma) = r_1 + \gamma \, g(t_{2:}, \gamma),
\]
...
The first one considered is the state-value function \(V\).  
It tells us how desirable a particular state is under a given policy \(\pi\), given an observation \(o\), the environment dynamics \(p\), and the discount factor \(\gamma\).
Strictly speaking, this function should be written as \(V(s, o, p, \gamma, \pi)\) to reflect all its dependencies.
However, since both the environment dynamics \(p\) and the discount factor \(\gamma\) are fixed throughout learning, they are omitted from the notation.
...
\[
V_\pi(s, o) = \mathbb{E}\Big[g(t, \gamma)\Big], \quad \text{with } t \sim \tau_t(s, o, p, \pi).
\]
...
\begin{aligned}
V_\pi(s, o) 
&= \mathbb{E}\Big[g(t, \gamma)\Big], \quad \text{with } t \sim \tau_t(s, o, p, \pi) \\[4pt]
&= \mathbb{E}\Big[r_1 + \gamma \, g(t_{2:}, \gamma)\Big] \\[4pt]
...
\begin{aligned}
V_\pi(s, o) 
&= \mathbb{E}\Big[r_1 + \gamma \, V_\pi(s_2)\Big] \\[4pt]
...
It is defined as:
\[
Q_\pi(s, a) = \mathbb{E}\Big[\, r + \gamma \, g(t, \gamma) \,\Big], \quad \text{with } t \sim \tau_t(s', o, p, \pi), \quad (s', o, r) \sim p(\cdot \mid s, a).
\]
...
\begin{aligned}
Q_\pi(s, a) 
&= \mathbb{E}\Big[\, r + \gamma \, g(t, \gamma) \,\Big], \quad \text{with } t \sim \tau_t(s', o, p, \pi), \quad (s', o, r) \sim p(\cdot \mid s, a) \\[4pt]
...
The goal of \gls{rl} is to find a policy that maximizes the expected cumulative reward from any observation until the end of the trajectory.
This policy is called the optimal policy and is denoted by \(\pi^*\).
In a partially observable setting, the optimal action is the one that maximizes the expected state-action value over the distribution of possible states given the observation:
\[
\pi^*(a \mid o) =
\begin{cases}
1 & \text{if } a = \underset{a' \in \mathcal{A}}{\arg\max}\;
\sum_{s \in \mathcal{S}} b(s \mid o)\, Q_{\pi^*}(s, a'), \\[6pt]
0 & \text{otherwise}
\end{cases}
\]

As shown, both \(V_\pi\) and \(Q_\pi\) can be expressed explicitly in terms of the environment dynamics \(p\) and the policy \(\pi\).
If \(p\), \(b\)  and \(\pi\) are fully known, one can theoretically compute the optimal policy by unfolding the graph of all possible trajectories starting from a given state \(s\) and observation \(o\).  
