...
Now that the main developments shaping reinforcement learning have been outlined, the formal definition of its objective can be introduced.
...
To specify the task and the agent's capacity to perceive and act, the problem is modeled as a Markov Decision Process (MDP).
...
Our focus here is to represent reinforcement learning as it is applied, even if this means sacrificing some mathematical proof capability.  
...
\begin{itemize}
    ...
    \item Finite-horizon MDPs are considered.  
...
Now that both the MDP and the policy \(\pi\) have been defined, the interaction between them can be characterized.  
