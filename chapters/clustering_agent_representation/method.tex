...
In this process, several surrogate \gls{nn} are independently trained in a supervised manner to reproduce the behavior of the same agent.
...
The encoder of each surrogate policy embeds the shared observation set $\mathbb{O}$ (Line~\ref{alg:car:embed}), and the resulting point cloud is partitioned by $f_{\mathrm{clust}}$ (Line~\ref{alg:car:clust}).
