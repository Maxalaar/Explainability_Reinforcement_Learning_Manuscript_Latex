...
Their \gls{lr} should therefore produce similar clusterings when analyzed using the same clustering procedure.
...
We consider $\mathcal{C}_{obs}$, the clustering of the observation space $\mathbb{O}$, and $\mathcal{C}_{act}$, the clustering of the action distribution of the initial policy.
Concretely, $\mathcal{C}_{act}$ is computed on the parameters of this distribution (the logits, before the softmax) produced by the initial policy for the same observations, which are precisely the quantities the surrogate policies are trained to reproduce.
...
\begin{align}
\scalebox{1}{%
$\vcenter{\hbox{\shortstack{\textit{Adjusted}\\\textit{Clustering}\\\textit{Universality}}}}
=
\vcenter{\hbox{\shortstack{\textit{Clustering}\\\textit{Universality}}}}
-
\frac{1}{n}\sum_{i=1}^{n}
\max\left(
\left|A^{obs}_{i}\right|,
\left|A^{act}_{i}\right|,
\left|A^{untr}_{i}\right|
\right)$%
}
\end{align}
