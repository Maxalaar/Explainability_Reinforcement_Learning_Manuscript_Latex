...
To evaluate the effectiveness of \gls{car}, three evaluation metrics are introduced:
...
This metric compares the returns obtained by the original policy $\OriginalPolicy$ and the surrogate policies $\SurrogatePolicy{}$ over a set of evaluation episodes.
...
We denote by $g_{\OriginalPolicy}^{i}$ the return obtained by the original policy in episode $i$, and by $g_{\SurrogatePolicy{j}}^{k}$ the return obtained by surrogate policy $j$ in episode $k$, with $i,k\in{1,\dots,m}$ and $j\in{1,\dots,n}$.
The \BehaviorReconstructionName{} is then defined as the mean return of the surrogate policies normalized by the mean return of the original policy:

\begin{align}
    \text{\BehaviorReconstructionName} = \frac{1}{n \sum_{i=1}^{m} g_{\OriginalPolicy}^{i}} \sum_{j=1}^{n} \sum_{k=1}^{m} g_{\SurrogatePolicy{j}}^{k}
\end{align}

where $n$ is the number of surrogate policies.
