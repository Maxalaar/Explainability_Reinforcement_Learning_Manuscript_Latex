...
This definition of explanation establishes the approach to explainability adopted throughout this manuscript.
Under this definition, explainability is regarded as a continuum that evolves as knowledge develops \cite{Siegel_Craver_2024, zednik2019solving}.

This quantitative perspective highlights that not all explanations provide the same level of value \cite{doshivelez2017rigorous}.
...
Unlike symbolic approaches, individual components of \gls{nn} cannot be directly associated with human concepts.
...
\begin{itemize}
    ...
    This corresponds to the ``black box'' view of \gls{nn}.
    ...
    \item \textbf{Intrinsic explainability:} In this approach, it is argued that \gls{nn} contain mechanisms that can be explained.
...
Although both approaches have produced convincing results, this manuscript primarily adopts the perspective of the \MechanisticHypothesisName \cite{Siegel_Craver_2024, zednik2019solving}.
This perspective is more closely aligned with the definition of explainability adopted in this manuscript, which considers explainability as a continuum that evolves with the development of knowledge.
...
Within intrinsic explainability, the \MechanisticHypothesisName underlies the approach that we refer to as \gls{me} (Definition~\ref{def:mechanistic_explainability}), generally called mechanistic interpretability in the literature \cite{olah2020zoom}.
The definitions introduced above clarify this terminological choice.
According to Definition~\ref{def:interpretable}, the internal mechanisms of a \gls{nn} are not interpretable, since an observer cannot directly understand them without an additional transformation process.
What this approach produces is interpretable information obtained from these mechanisms through an explainability method (Definition~\ref{def:explainability_method}), that is, explanations in the sense of Definition~\ref{def:explanation}.
It therefore falls within explainability rather than interpretability.
