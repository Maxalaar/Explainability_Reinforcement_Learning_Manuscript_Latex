...
We refer to this process as an explainability method (Definition~\ref{def:explainability_method}).
...
\begin{enumerate}
    ...
    \item[(2)] The \textbf{explanation} itself, the interpretable information produced from the object.
    This transformation is labeled information extraction, and is performed through the use of an explainability method (Definition~\ref{def:explainability_method}), detailed further in Section~\ref{subsec:explainability_method}.
    \item[(3)] The \textbf{mental model} held by the observer regarding the object.
    This transformation is labeled interpretation by the observer, corresponding to the cognitive step through which the observer refines their mental model upon receiving the explanation.
\end{enumerate}
...
In our framework, an explanation in the field of \gls{xai} can be characterized by three components \cite{guidotti2018survey}:
\begin{itemize}
...
These three aspects of an explanation are entirely determined by the explainability method (Definition~\ref{def:explainability_method}) used to produce it.
