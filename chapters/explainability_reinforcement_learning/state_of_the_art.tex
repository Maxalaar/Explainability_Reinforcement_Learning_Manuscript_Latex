...
\input{figures/explainability_reinforcement_learning/taxonomy_table.tex}
...
Methods belonging to the same family may differ in their implementation or in the details of the generation process, while relying on the same underlying principle.

The remainder of this section is organized around the six forms of explanation identified in Section~\ref{subsec:form}.
Each subsection focuses on one form of explanation, with its subsubsections presenting the corresponding families of methods.
...
In modern \gls{rl}, \gls{nn} is the most commonly used models for representing policies.
...
This family of methods aims at learning a policy represented by an interpretable model \cite{topin2021iterative, trivedi2022learning}.
...
An interpretable model can be trained to approximate a policy learned through deep \gls{rl} \cite{sieusahai2021explaining}.
In this setting, the problem is formulated as a supervised learning task, where the original deep \gls{rl} policy is used to label each observation with the action it selects, such as the observation from the Atari game ``Pong'' shown in Figure~\ref{fig:policy_approximation_observation}.
Using this dataset, the interpretable model is trained to reproduce the behavior of the original policy, yielding for instance the decision tree illustrated in Figure~\ref{fig:policy_approximation_decision_tree}, obtained by approximating a deep \gls{rl} policy trained on that environment.
The resulting interpretable model is then used as an explanation for the observer.
If this approximation is sufficiently faithful, it can also be deployed in the environment in place of the original policy, thereby providing a high degree of control over the agent.

\input{figures/explainability_reinforcement_learning/policy_approximation_example.tex}
...
In Figure~\ref{fig:saliency_map_heatmap}, both panels correspond to the same observation of the Atari game ``Breakout'', with panel~(a) obtained by observation perturbation and panel~(b) by observation gradient analysis.

\input{figures/explainability_reinforcement_learning/saliency_map_heatmap.tex}
...
Perturbation analysis consists in applying various modifications to the observation in order to study their impact on the output of the policy function \cite{greydanus2018visualizing}.
...
Observation gradient analysis refers to a set of methods that require a differentiable policy function in order to extract information from it \cite{simonyan2014deep, weitkamp2019visual, huber2019enhancing}.
...
Attention blocks constitute a \gls{nn} architecture that makes it possible to weight the relative importance of one part of a vector with respect to the other parts of the same vector \cite{vaswani2023attention}.
...
This attention-based \textit{heatmap} is then provided to the observer as the explanation, as illustrated for the Atari game ``Breakout'' in Figure~\ref{fig:attention_map_heatmap_clear}, where the highlighted regions are easily interpretable.
However, as shown for ``Ms.\ Pac-Man'' in Figure~\ref{fig:attention_map_heatmap_unclear}, the attention weights are not always concentrated on a small, easily interpretable set of regions, which can make this type of explanation difficult to exploit.

\input{figures/explainability_reinforcement_learning/attention_map_heatmap.tex}

\subsection{Counterfactual}
\label{subsec:counterfactual}

In the context of \gls{rl}, a counterfactual explanation consists in generating a new observation, distinct from the original one, that leads the agent to select a different action.
By comparing the original and the modified observations, the observer can identify which elements of the environment must change for the agent to adopt an alternative behavior.
This form of explanation therefore provides insight into the agent's decision-making process.
The family of methods used to produce such explanations relies on the \textit{use of the \gls{ls} for counterfactual generation}.
...
This family of methods employs generative models \cite{bond2021deep} to exploit the \gls{lr}.
A generative model learns to produce new data by imitating the distribution of its training data.
Here, it is used to synthesize alternative observations that are likely to change the agent's action \cite{olson2021counterfactual}.

\input{figures/explainability_reinforcement_learning/counterfactual_example.tex}

Figure~\ref{fig:counterfactual_example} illustrates this approach on the Atari game \textit{Space Invaders}.
Figure~\ref{fig:counterfactual_example_original} shows the original observation, for which the agent selects ``LEFT''.
Figure~\ref{fig:counterfactual_example_counterfactual} shows the counterfactual observation produced by the generative model, for which the agent instead selects ``RIGHT\_FIRE''.
Comparing the two panels reveals some of the factors that influence the agent's decision.
In Figure~\ref{fig:counterfactual_example_original}, the agent does not fire because a projectile is already present on the screen, which makes it impossible to launch another one.
The transition to Figure~\ref{fig:counterfactual_example_counterfactual} also suggests that the agent targets the lowest enemy first, since removing that enemy causes the agent to redirect its shot toward the next lowest one.

These observations should not be interpreted as definitive evidence of the agent's strategy.
Such behaviors would need to be observed repeatedly across many different situations before drawing stronger conclusions.
Nevertheless, counterfactual explanations can provide useful clues about the factors considered by the agent and help an observer develop a better understanding of its behavior.

The main challenge of this approach lies in the fact that, to serve as explanations, counterfactuals must be both close and feasible.
A close counterfactual is one that does not differ significantly from the original observation.
A feasible counterfactual corresponds to an observation that can actually occur in the considered environment.
...
In this family of methods, the predictions of the critic are used to determine which trajectories to present to the observer \cite{Sequeira_2020}.
...
This family of methods distributes the optimization of the reward function across multiple models, significantly simplifying the learning process.
Moreover, this decomposition of the policy function can also provide explainability in the agent's strategy \cite{Beyret_2019}.
...
To improve the interpretability of value-based explanations, a family of methods known as \textit{critic enrichment} can be employed.

\subsubsection{\textit{Critic Enrichment}}
\label{subsubsec:critic_enrichment}

In order to obtain interpretable information from the predictions of the critic, it is possible to decompose the critic into multiple sub-functions \cite{juozapaitis2019explainable}.
...
\input{figures/explainability_reinforcement_learning/trajectory_prediction_values.tex}
...
The evolution of an agent within an environment can be observed as a set of random variables connected through a Bayesian network \cite{madumal2020explainable}.
...
At the end of this process, a weighted causal graph is obtained, which serves as an explanation for the observer, as illustrated for the video game \textit{StarCraft~II} in Figure~\ref{fig:interaction_model_causal_graph}, where nodes represent random variables and edges represent causal relationships between them.
...
\input{figures/explainability_reinforcement_learning/interaction_model_causal_graph.tex}

\subsubsection{\textit{Latent Space Usage for MDP Generation}}
\label{subsubsec:latent_space_mdp_generation}

Each layer of a \gls{nn} can be viewed as a projection from one space to another \cite{zahavy2017graying}.
...
An agreement can be observed between the structure of this projected space and the critic's predictions: states that are assigned similar values by the critic also tend to be located close to one another in the projection.
...
\input{figures/explainability_reinforcement_learning/latent_space_tsne_example.tex}
