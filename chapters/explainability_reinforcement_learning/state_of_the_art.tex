...
\input{figures/explainability_reinforcement_learning/xrl_methods_overview.tex}

The purpose of this section is to detail the functioning of existing explainability methods (see Table~\ref{tab:taxonomy_methods} and Figure~\ref{fig:xrl_methods_overview}).
...
Each subsection introduces one of the six forms of explanation identified in Section~\ref{subsec:form}, as well as the methods associated with it.

\subsection{Policy Model}
\label{subsec:policy_model}

There exists a category of models that can be understood through the simple analysis of their representation; such models are referred to as interpretable \cite{bellucci2021towards}.
These models are used to represent the policy function.
The main models considered interpretable include decision trees \cite{topin2021iterative}, algorithms \cite{trivedi2022learning}, formal logic \cite{payani2019inductive}, and linear models \cite{pmlr-v108-garreau20a}.
The intrinsic interpretability of these models can be leveraged to provide explainability in the context of reinforcement learning (RL).

This directly interpretable representation allows an observer to extract information.
...
Using this set of weights, a \textit{heatmap} is constructed, enabling the observer to visualize which parts of the observation vector are most sensitive in the action-selection process.
...
Based on these weightings, a \textit{heatmap} is constructed to allow the observer to visually identify the parts of the input vector that are the most influential in the policy function's output.
...
This attention-based \textit{heatmap} is then provided to the observer as the explanation.
