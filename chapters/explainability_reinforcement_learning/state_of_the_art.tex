...
Within each subsection, each subsubsection is dedicated to a family of methods capable of producing that form of explanation.
...
In modern \gls{rl}, \glspl{nn} is the most commonly used models for representing policies.
...
In this setting, the problem is formulated as a supervised learning task, where the original deep \gls{rl} policy is used to label each observation with the action it selects (see Figure~\ref{fig:policy_approximation_observation}).
Using this dataset, the interpretable model is trained to reproduce the behavior of the original policy, yielding for instance the decision tree illustrated in Figure~\ref{fig:policy_approximation_decision_tree}.
...
In the context of \gls{rl}, counterfactual explanations are used to generate new observations.
...
The family of methods available to generate counterfactuals is the \textit{use of the \gls{ls} for counterfactual generation}.
...
In this family of methods, \textbf{generative models} are used \cite{bond2021deep} to exploit the \gls{lr}.
This type of model learns to generate new data by imitating the training data distribution.
In this context, generative models aim to produce alternative observations that are likely to alter the agent's actions (see Figure~\ref{fig:counterfactual_example}) \cite{olson2021counterfactual}.
...
Using this approach, for each observation and action, a prediction is obtained regarding the sub-objectives the agent is attempting to maximize (see Figure~\ref{fig:trajectory_prediction_values}).
...
At the end of this process, a weighted causal graph is obtained, which serves as an explanation for the observer (see Figure~\ref{fig:interaction_model_causal_graph}).
...
Each point is colored according to the value function's prediction for the corresponding state.
