...
The method available to generate counterfactuals is the \textit{use of the \gls{ls} for counterfactual generation}.
...
In this method, \textbf{generative models} are used \cite{bond2021deep} to exploit the \gls{lr}.
...
This simple approach can be further enhanced using methods such as \textit{hierarchical decomposition} and \textit{extraction via critic analysis}.
...
To improve the interpretability of value-based explanations, a technique known as \textit{critic enrichment} can be employed.
...
The methods used to obtain explanations in the form of agent--environment interactions are: \textit{Bayesian network learning} and \textit{\gls{ls} usage for \gls{mdp} generation}.
