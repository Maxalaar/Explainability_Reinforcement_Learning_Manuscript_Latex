...
Using an interpretable model, it is possible to approximate a policy represented by a deep neural network trained with deep reinforcement learning \cite{sieusahai2021explaining}.
...
It provides a simple way to verify that the agent behaves as expected and to detect potential implementation errors, such as incorrect environment dynamics or reward functions.
Furthermore, episode visualization provides an intuitive overview of the agent's strategy and often constitutes the first form of analysis performed by practitioners.
...
Figure~\ref{fig:latent_space_tsne_example} illustrates this idea using a two-dimensional \textit{t-SNE} projection of the latent space, where each point is colored according to the value function's prediction for the corresponding state.
