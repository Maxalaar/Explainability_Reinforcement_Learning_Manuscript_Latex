...
A limitation of heatmap-based methods is that the resulting explanations can vary significantly depending on the method used to generate them.
This highlights a critical challenge in XAI: determining whether the produced explanations are relevant and how to choose between different explanations that may appear contradictory.
Rather than considering these differences as evidence that only one explanation can be valid, the definition of explanation adopted in this manuscript allows for multiple complementary perspectives.
Different explanations may capture different aspects of the system and contribute to improving the observer's understanding from different viewpoints.
