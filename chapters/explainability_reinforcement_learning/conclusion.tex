...
Many existing methods explain \gls{rl} agents by constructing surrogate models.
While these approaches can provide useful explanations for simple policies, their ability to scale to increasingly complex agents is limited.
This limitation is particularly important in deep \gls{rl}, where \gls{nn} are used to represent complex policies that would be difficult to approximate with simpler surrogate models.
Replacing such a model with a simpler one therefore risks losing precisely the complexity that makes deep \gls{rl} effective.

For this reason, we argue that a promising direction for \gls{xrl} is to develop explanations directly from the \gls{nn} underlying the agent.
Rather than bypassing the model, such approaches aim to analyze the representations learned by the network itself.
From this perspective, the \gls{nn} should not necessarily be regarded as an inherently opaque black box, but as a complex representation whose apparent opacity may instead reflect the limitations of current analysis tools.
...
Instead of replacing the learned representation, it directly exploits the \gls{ls} to identify meaningful concepts that can support the explanation of the agent's behavior.
