...
This section complements them with a qualitative, hand-labeled inspection of the clusters obtained for each environment, in the spirit of Figure~\ref{fig:semantic_granularity}.
For each environment, $k$-means is applied to a fixed layer and number of clusters $K$, chosen for the interpretability of the resulting groupings rather than for the highest \gls{acu}, and a sample of observations from each cluster is inspected to propose a semantic label.
...
Unlike Mario Bros, the clustering here separates the coarse temporal phase of the episode, tracked through the score and the number of pellets remaining in the maze, rather than a spatial feature of the observation.
