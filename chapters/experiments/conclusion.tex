...
Across the six environments, the \gls{cu} frequently exceeds $0.5$, indicating that independently trained policies can develop latent representations with sufficiently similar structural organization \cite{amezquita2020orchestrating}.
A consistent trend in \gls{cu} is observed across network depth, with values generally decreasing in deeper layers.
...
This highlights the importance of considering both consistency and abstraction when identifying meaningful latent representations.
...
The qualitative analysis complements these quantitative findings by showing that the clusters identified by \gls{car} correspond to interpretable properties of the environment and of the agent's behavior.
...
As the clusters become more fine-grained, the semantic boundaries between neighboring situations become less clearly defined, making independently trained policies less likely to assign the same observations to identical clusters.
...
At the same time, the qualitative correspondence between clusters and meaningful properties of the environment.
