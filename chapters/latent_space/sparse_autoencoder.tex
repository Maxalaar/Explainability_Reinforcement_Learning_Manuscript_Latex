...
As a result, in a superposed \gls{lr}, linear analysis of the \gls{ls} may not isolate individual concepts, the resulting directions reflect a mixture of several semantic attributes.
...
\begin{itemize}
    ...
    \item $\gamma \|\latentRepresentation - \hat{\latentRepresentation}\|_2^2$ is the reconstruction loss ensuring that the original \gls{lr} can be faithfully recovered,
...
In practice, both $f_{\text{enc}}$ and $f_{\text{dec}}$ are implemented as linear projections between the original \gls{ls} and the high-dimensional space $\mathcal{Z}$.
This design follows the same principle as in probing methods, the transformation used to analyze the \gls{lr} should remain as simple as possible.
More expressive projection functions could themselves introduce complex structures into the transformed \gls{lr}.
