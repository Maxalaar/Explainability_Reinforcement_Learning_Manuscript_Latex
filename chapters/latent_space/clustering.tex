...
All the \gls{ls} analysis methods presented so far rely on assumptions about a linear \gls{cs}.For probing methods, concepts must be at least partially linearly separable for simple probes to succeed \ref{sec:probing}.
Latent directions assumes that variations associated with a concept can be captured by moving along a linear axis of the \gls{ls} \ref{sec:direction}.
Similarly, the superposition hypothesis (Hypothesis~\ref{hyp:superposition}) assumes that concepts are represented through linear directions that become entangled because of representational capacity constraints (Section~\ref{sec:sparse_autoencoder}).
Although these assumptions have led to empirical successes, their theoretical justification remains limited.
Several observations suggest that the linearity hypothesis may not fully capture the \gls{cs}.
