...
They investigate whether the presence or absence of a selected concept can be inferred from embeddings.
...
For each example $i$, let $h_l^{(i)} \in \mathbb{R}^{n_l}$ denote the representation vector obtained at layer $l$.
A classifier, referred to as a probe, is then trained to predict the presence of a given concept from this representation.
The probe is denoted by $p_{\theta}^{l,j}$, where $\theta$ represents its parameters, $l$ identifies the layer from which the representation is extracted, and $j$ identifies the concept being predicted.
It maps the representation $h_l^{(i)}$ to a scalar value in $[0,1]$, which can be interpreted as the probability that the concept is present.
