...
To do so, the average loss on the training set, denoted $\bar{\mathcal{L}}_{\text{train}}$, is studied. This is an instance of the performance measure $P$ defined in Equation~\eqref{eq:performance_measure}, restricted to $\mathcal{D}_{\text{train}}$:
\begin{equation}
\bar{\mathcal{L}}_{\text{train}} = \frac{1}{|\mathcal{D}_{\text{train}}|} \sum_{(x^{(i)}, y^{(i)}) \in \mathcal{D}_{\text{train}}} \fullLoss{\theta}{i}.
\label{eq:train_loss}
\end{equation}

If $\bar{\mathcal{L}}_{\text{train}}$ is not sufficiently low, this means that the model has not been able to learn the task.
...
\begin{equation}
\bar{\mathcal{L}}_{\text{test}} = \frac{1}{|\mathcal{D}_{\text{test}}|} \sum_{(x^{(i)}, y^{(i)}) \in \mathcal{D}_{\text{test}}} \fullLoss{\theta}{i}.
\label{eq:test_loss}
\end{equation}

If $\bar{\mathcal{L}}_{\text{test}} \gg \bar{\mathcal{L}}_{\text{train}}$, the model fails to generalize.
...
If $\bar{\mathcal{L}}_{\text{test}} \approx \bar{\mathcal{L}}_{\text{train}}$, this indicates that the training procedure has successfully produced a \gls{nn} that generalizes well beyond the training data \cite{Kawaguchi_2022}.
