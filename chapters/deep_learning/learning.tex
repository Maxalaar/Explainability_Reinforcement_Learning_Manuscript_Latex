...
\end{equation}
For conciseness, the per-example loss is denoted $\ell^{(i)}(\theta) \overset{\mathrm{def}}{=} \mathcal{L}\left(\hat{f}_{\theta}(x^{(i)}), y^{(i)}\right)$ throughout the remainder of this section, so that the training objective can equivalently be written as $\min_{\theta} \frac{1}{N} \sum_{i=1}^{N} \ell^{(i)}(\theta)$.
...
The effect of each parameter $w_k$ on the loss $\ell^{(i)}(\theta)$ is quantified by the corresponding partial derivative
$\frac{\partial \ell^{(i)}(\theta)}{\partial w_k}$.
...
\left[
\frac{\partial \ell^{(i)}(\theta)}{\partial w_1},
\dots,
\frac{\partial \ell^{(i)}(\theta)}{\partial w_P}
\right].
\label{eq:gradient}
\end{equation}
