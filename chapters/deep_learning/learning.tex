...
In \gls{dl}, the loss depends on the network parameters $\theta$ only indirectly, through its predictions, and $\theta$ can contain millions or even billions of parameters, resulting in a much higher-dimensional and complex loss landscape, which is typically non-convex.
Figure~\ref{fig:multiple_gradient_descents_non_convex} illustrates the effect of this non-convexity on standard non-convex functions, where different starting points lead to distinct local minima, unlike the single global minimum reached in the convex example above.
