...
The effect of each parameter $w_k$ on the loss $\mathcal{L}(\hat{f}_{\theta}(x^{(i)}), y^{(i)})$ is quantified by the corresponding partial derivative
...
Each layer function $\phi^{(l)}$ is itself parameterized by the weights belonging to layer $l$, so it can equally be viewed as a function of these parameters; this is what makes it possible, later on, to differentiate $\phi^{(l)}$ with respect to $w_k$.
...
Consider now two consecutive layers, $\phi^{(l)}$ followed by $\phi^{(l+1)}$, so that $\mathbf{h}^{(l+1)} = \phi^{(l+1)}\big(\phi^{(l)}(\mathbf{h}^{(l-1)})\big)$.
...
Once $\frac{\partial \mathcal{L}(\hat{f}_{\theta}(x^{(i)}), y^{(i)})}{\partial w_k}$ can be computed for every parameter, all of these values are stacked into one vector, called the gradient:
\begin{equation}
