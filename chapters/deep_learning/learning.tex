...
\end{equation}
For conciseness, the per-example loss incurred on example $i$ by the parameters $\theta$ is denoted $\ell^{(i)} \overset{\mathrm{def}}{=} \mathcal{L}\left(\hat{f}_{\theta}(x^{(i)}), y^{(i)}\right)$ throughout the remainder of this section. This quantity is a scalar, and its dependence on $\theta$ is left implicit to keep the notation light. The training objective can then equivalently be written as $\min_{\theta} \frac{1}{N} \sum_{i=1}^{N} \ell^{(i)}$.
...
The effect of each parameter $w_k$ on the loss $\ell^{(i)}$ is quantified by the corresponding partial derivative
$\frac{\partial \ell^{(i)}}{\partial w_k}$.
...
Once $\frac{\partial \ell^{(i)}}{\partial w_k}$ can be computed for every parameter, all of these values are stacked into one vector, called the gradient:
\begin{equation}
\nabla_{\theta} \ell^{(i)} =
\left[
\frac{\partial \ell^{(i)}}{\partial w_1},
\dots,
\frac{\partial \ell^{(i)}}{\partial w_P}
\right].
\label{eq:gradient}
\end{equation}
...
\end{equation}
where $\eta > 0$ is the learning rate, which controls the step size, and $\nabla_{\theta^{(n)}} \ell^{(i)}$ denotes the gradient of the loss with respect to the parameters, evaluated at the current values $\theta^{(n)}$.
