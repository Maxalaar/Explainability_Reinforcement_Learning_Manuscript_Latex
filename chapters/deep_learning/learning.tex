...
To do so, the effect of each parameter $w_k$ on the loss $\mathcal{L}(\hat{f}_{\theta}(x^{(i)}), y^{(i)})$ is quantified by the corresponding partial derivative
...
\end{equation}
and, since this function is itself parameterized by the weights belonging to layer $l$, it can equally be seen as taking these parameters as an additional input.
With this view, the whole network is nothing more than the composition of its $L$ layer functions, $\hat{f}_{\theta} = \phi^{(L)} \circ \cdots \circ \phi^{(1)}$.
...
\begin{equation}
J_{\mathbf{f} \circ \mathbf{g}}(\mathbf{x}) = J(f(g(x))) = J_{\mathbf{f}}(\mathbf{g}(\mathbf{x})) \, J_{\mathbf{g}}(\mathbf{x}).
