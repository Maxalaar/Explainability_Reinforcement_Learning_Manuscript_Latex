...
To simplify the presentation throughout this chapter, we focus on the \gls{mlp} architecture.
...
\input{figures/deepl_learning/artificiale_neurone.tex}
...
In \gls{mlp} architectures, neurons are organized into successive layers, as illustrated in Figure~\ref{fig:mlp_architecture}.
...
% \begin{figure}[t]
% \centering
% \includegraphics[width=0.55\linewidth]{../../figures/deepl_learning/mlp_architecture.pdf}
% \caption{Representation of the \gls{mlp} architecture}
...
\item one or more \emph{hidden layers}, which perform intermediate transformations,
...
This matrix-vector representation is visualised in Figure~\ref{fig:interaction_between_layers}, which highlights how each neuron in layer $l$ receives weighted signals from all neurons in the previous layer.

% \begin{figure}[t]
% \centering
% \includegraphics[width=0.65\linewidth]{../../figures/deepl_learning/interaction_between_layers.pdf}
% \caption{Representation of the interaction between layer $l-1$ and layer $l$}
% \label{fig:interaction_between_layers}
% \end{figure}
...
An entire chapter of this manuscript is devoted to studying the information that can be extracted from the activations of these intermediate layers.
...
In this manuscript, we consider $\theta$ as a vector in $\mathbb{R}^P$, where $P$ denotes the total number of parameters $\theta = [w_1, \dots, w_P]$.
...
When we refer to the architecture of $\hat{f}$, we mean the number of layers, the number of neurons per layer, and the choice of activation functions.
