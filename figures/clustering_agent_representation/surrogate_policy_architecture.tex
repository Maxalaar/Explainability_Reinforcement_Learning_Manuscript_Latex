\begin{figure}[t]
    ...
    \node[align=center] (encoder_label) at ($(encoder_box.north) + (0, 0.5cm)$) {\scalebox{1.5}{$\textcolor{myblue}{f_{enc}}$}};
    ...
    \node[align=center] (decoder_label) at ($(decoder_box.north) + (0, 0.5cm)$) {Decoder};
    ...
    \caption{Architecture of the surrogate policy $\SurrogatePolicy{}$. The blue dashed box denotes the encoder $\textcolor{myblue}{f_{enc}}$, which maps the observation $O$ to the embedding, and the black dashed box denotes the decoder, which reconstructs the action distribution parameters from it. Both boxes encapsulate the embedding layer, illustrating that the output of the encoder is the input of the decoder.}
