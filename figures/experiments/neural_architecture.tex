...
% --- Big "A" bubble ---------------------------------------------------
% Anchor = the top-right corner of box A.
\providecommand{\NeuralArchLabelABubbleOffsetX}{0cm}
\providecommand{\NeuralArchLabelABubbleOffsetY}{0cm}
\providecommand{\NeuralArchLabelATextOffsetX}{-0.1cm}
\providecommand{\NeuralArchLabelATextOffsetY}{0cm}

% --- Big "B" bubble ---------------------------------------------------
% Anchor = the top-right corner of box B.
\providecommand{\NeuralArchLabelBBubbleOffsetX}{0cm}
\providecommand{\NeuralArchLabelBBubbleOffsetY}{0cm}
\providecommand{\NeuralArchLabelBTextOffsetX}{-0.1cm}
\providecommand{\NeuralArchLabelBTextOffsetY}{0cm}

% --- Size shared by both big bubbles (A and B) -----------------------
...
% #1 = anchor coordinate        #2 = bubble offset (X,Y)   #3 = bubble size
% #4 = text offset (X,Y)        #5 = text font              #6 = text content
\newcommand{\NeuralArchBubble}[6]{%
    \node[bubbleDraw, minimum size=#3] at ($(#1)+(#2)$) {};
    \node[font=#5, inner sep=0pt, anchor=center] at ($(#1)+(#4)$) {#6};
}
...
\NewDocumentCommand{\NeuralArchNumLabel}{
    ...
    m m}{%
    \NeuralArchBubble{#3-north}{#1}{\NeuralArchNumBubbleSize}{#2}{\NeuralArchNumFont}{#4}%
}

% Big "A"/"B" bubble, anchored on a given box corner.
% #1 = anchor coordinate (e.g. boxA.north east)
% #2 = (X,Y) bubble offset   #3 = (X,Y) text offset   #4 = letter (A or B)
\newcommand{\NeuralArchBigLabel}[4]{%
    \NeuralArchBubble{#1}{#2}{\NeuralArchLetterBubbleSize}{#3}{\NeuralArchLetterFont}{\NeuralArchCircleLetter{#4}}%
}
...
% Label A + bubble
% ------------------------------------------------------------
% Anchor = coin HAUT-DROIT (north east) de la boîte A. Les offsets A
% (voir section Paramètres/Bulles) déplacent la bulle et le texte
% indépendamment par rapport à ce coin.

\NeuralArchBigLabel{boxA.north east}
    {\NeuralArchLabelABubbleOffsetX,\NeuralArchLabelABubbleOffsetY}
    {\NeuralArchLabelATextOffsetX,\NeuralArchLabelATextOffsetY}
    {A}
...
% Label B + bubble
% ------------------------------------------------------------
% Idem pour le label B : ancre = coin HAUT-DROIT de la boîte B.

\NeuralArchBigLabel{boxB.north east}
    {\NeuralArchLabelBBubbleOffsetX,\NeuralArchLabelBBubbleOffsetY}
    {\NeuralArchLabelBTextOffsetX,\NeuralArchLabelBTextOffsetY}
    {B}
...
\tikzset{
    legendBubble/.style={
        bubbleDraw,
        minimum size=\NeuralArchLetterBubbleSize,
        font=\NeuralArchLetterFont
    }
}
...
\node[
    ...
    \tikz[baseline=(legendA.base)]{
        \node[legendBubble] (legendA) {\NeuralArchCircleLetter{A}};
    }
    ...
    \tikz[baseline=(legendB.base)]{
        \node[legendBubble] (legendB) {\NeuralArchCircleLetter{B}};
    }
