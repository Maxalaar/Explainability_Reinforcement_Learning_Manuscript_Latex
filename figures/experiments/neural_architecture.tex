...
% Offset of the label/circle A relative to the top-left corner of box A
\providecommand{\NeuralArchLabelAOffsetX}{0.0cm}
\providecommand{\NeuralArchLabelAOffsetY}{0.0cm}

% Offset of the label/circle B relative to the top-left corner of box B
\providecommand{\NeuralArchLabelBOffsetX}{0.1cm}
\providecommand{\NeuralArchLabelBOffsetY}{0.1cm}
...
% Flatten
% --------------------------------------------------------
% Drawn as a single wide dense bar (same shape convention as the
% Dense boxes below) so the flattening operation reads clearly as
% "all conv features concatenated into one vector".

\pic[shift={(1.3,0,0)}] at (c4-east)
    {Box={
        name=flatten,
        caption={\mbox{Flatten}},
        fill=FlattenColor,
        opacity=0.85,
        height=3,
        width=2,
...
% Box A
% ------------------------------------------------------------
% inner xsep/ysep on a \tikz fit node are symmetric, so an
% asymmetric top/bottom margin needs the box built in two steps:
% first fit tightly around the content, then grow that fit by
% independent amounts on each side before drawing the outline.
...
% Label A + circle
% ------------------------------------------------------------
% A single auto-sizing circle node: its radius follows the size of
% the "A" glyph plus \NeuralArchCircleInnerSep, and its position is
% freely adjustable via the \NeuralArchLabelAOffsetX/Y parameters.

\node[
    archLabelCircle,
    anchor=south east
] (labelA) at
    ($(boxA.north west)+(\NeuralArchLabelAOffsetX,\NeuralArchLabelAOffsetY)$)
...
\node[
    ...
    ($(boxB.north west)+(\NeuralArchLabelBOffsetX,\NeuralArchLabelBOffsetY)$)
